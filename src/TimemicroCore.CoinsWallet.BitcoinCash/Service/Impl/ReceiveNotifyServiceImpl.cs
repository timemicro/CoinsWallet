...
namespace TimemicroCore.CoinsWallet.BitcoinCash.Service.Impl
{
    public class ReceiveNotifyServiceImpl : IReceiveNotifyService
    {
        ...
        public void Notify()
        {
            ...
            var result = new BCHReceiveNotifyResult()
            {
                Data = new List<BCHReceiveNotifyResultDataItem>()
            };

            foreach (var item in logs)
            {
                result.Data.Add(new BCHReceiveNotifyResultDataItem()
                {
            ...
            var http = WebRequest.CreateHttp(configuration["CoinsWallet:BitcoinCash:ReceiveNotifyUrl"]);
            try
            {

...
namespace TimemicroCore.CoinsWallet.Bitcoin.Service.Impl
{
    public class SendNotifyServiceImpl : ISendNotifyService
    {
        ...
        public void Notify()
        {
            ...
            var result = new BTCSendNotifyResult()
            {
                Data = new List<BTCSendNotifyResultDataItem>()
            };

            foreach (var item in logs)
            {
                result.Data.Add(new BTCSendNotifyResultDataItem()
                {
            ...
            var http = WebRequest.CreateHttp(configuration["CoinsWallet:Bitcoin:SendNotifyUrl"]);
            try
            {

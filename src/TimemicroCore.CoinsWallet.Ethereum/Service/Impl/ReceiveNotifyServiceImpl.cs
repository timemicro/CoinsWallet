...
namespace TimemicroCore.CoinsWallet.Ethereum.Service.Impl
{
    public class ReceiveNotifyServiceImpl : IReceiveNotifyService
    {
        ...
        public void Notify()
        {
            ...
            var result = new ETHReceiveNotifyResult()
            {
                Data = new List<ETHReceiveNotifyResultDataItem>()
            };

            foreach (var item in logs)
            {
                result.Data.Add(new ETHReceiveNotifyResultDataItem()
                {
            ...
            var http = WebRequest.CreateHttp(configuration["CoinsWallet:ethereum:ReceiveNotifyUrl"]);
            try
            {
